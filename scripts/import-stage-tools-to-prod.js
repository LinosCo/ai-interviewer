...
async function copyTable({ src, dst, table, pk }) {
  ...
  if (!sourceRows) return { sourceRows: 0, insertedRows: 0, skippedRows: 0 };
  ...
  for (const row of srcRowsRes.rows) {
    ...
    if (ins.rowCount > 0) insertedRows += 1;
    else skippedRows += 1;
  }
...
async function main() {
  ...
  if (!stageUrl) throw new Error('Missing STAGING_DATABASE_URL');
  if (!prodUrl) throw new Error('Missing PROD_DATABASE_URL (or DATABASE_URL)');
